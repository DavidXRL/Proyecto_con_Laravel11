        ...
        <div class="bg-white border border-red-200 rounded-2xl shadow-2xl w-full max-w-3xl p-6 sm:p-10 space-y-8">
            ...
            <div class="flex justify-center">
                <img src="/image/workshops_activities/{{ $workshop_activity->image_activity }}"
                     ...
                     class="w-full max-w-xl h-auto object-contain rounded-xl border-4 border-red-300 shadow-lg transition-transform duration-300 hover:scale-105">
            </div>
            <!-- Información detallada -->
            <div class="bg-gray-50 w-full rounded-xl p-6 shadow-inner space-y-3 text-gray-800">
                <p><span class="font-semibold text-gray-900">Nombre:</span> {{ $workshop_activity->name_workshop_activity }}</p>
                <p><span class="font-semibold text-gray-900">Descripción:</span> {{ $workshop_activity->description_workshop_activity }}</p>
                <p><span class="font-semibold text-gray-900">Fecha de publicación:</span> {{ \Carbon\Carbon::parse($workshop_activity->date_workshop_activity)->format('d/m/Y') }}</p>
                <p>
                    <span class="font-semibold text-gray-900">Hora de publicación:</span>
                    {{ $workshop_activity->time_workshop_activity ? \Carbon\Carbon::parse($workshop_activity->time_workshop_activity)->format('H:i') : 'Sin hora' }}
                </p>
            </div>
            <!-- Botones -->
            <div class="flex flex-col sm:flex-row justify-center gap-4 pt-4">
                <!-- Cancelar -->
                <form action="{{ route('workshops_activities.index') }}" class="w-full sm:w-auto">
                    <button type="submit"
                ...
                <!-- Eliminar -->
                <form action="{{ route('workshops_activities.destroy', $workshop_activity->id) }}" method="POST" class="w-full sm:w-auto">
                    @csrf
                    @method('DELETE')
                    <button type="submit"

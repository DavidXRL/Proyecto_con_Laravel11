...
<head>
    <meta charset="UTF-8" />
    <title>Lista de actividades | Telesecundaria Justo Sierra</title>
    ...
    <main class="max-w-5xl mx-auto mt-14 px-6 py-10">
        ...
        @if ($activities->isEmpty())
            <p class="text-center text-gray-500 text-lg py-20">No hay actividades registradas.</p>
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                @foreach ($activities as $activity)
                    <article
                        class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden flex flex-col">
                        <img src="/image/activities/{{ $activity->image_activity }}" alt="Imagen de actividad"
                        ...
                        <div class="p-6 flex flex-col flex-grow">
                            ...
                            <div class="flex gap-3 justify-end">
                                ...
                                <form action="{{ route('activities.delete', $activity) }}" method="POST" class="inline-block"
                                    >
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
